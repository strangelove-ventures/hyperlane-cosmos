...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewSimApp(log.NewNopLogger(), newDB, nil, true, wasm.EnableAllProposals, appOptions, emptyWasmOpts, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func setupSimulationApp(t *testing.T, msg string) (simtypes.Config, dbm.DB, simtestutil.AppOptionsMap, *SimApp) {
	...
	app := NewSimApp(logger, db, nil, true, wasm.EnableAllProposals, appOptions, emptyWasmOpts, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			db := dbm.NewMemDB()
			app := NewSimApp(logger, db, nil, true, wasm.EnableAllProposals, appOptions, emptyWasmOpts, interBlockCacheOpt(), baseapp.SetChainID(SimAppChainID))
