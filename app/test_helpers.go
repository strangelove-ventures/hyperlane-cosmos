...
func setup(t testing.TB, chainID string, withGenesis bool, invCheckPeriod uint, opts ...wasm.Option) (*SimApp, GenesisState) {
	db := dbm.NewMemDB()
	nodeHome := t.TempDir()
	...
	app := NewSimApp(log.NewNopLogger(), db, nil, true, wasmtypes.EnableAllProposals, appOptions, opts, bam.SetChainID(chainID), bam.SetSnapshot(snapshotStore, snapshottypes.SnapshotOptions{KeepRecent: 2}))
	if withGenesis {
...
func NewSimAppWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *SimApp {
	...
	app := NewSimApp(options.Logger, options.DB, nil, true, wasmtypes.EnableAllProposals, options.AppOpts, options.WasmOpts)
...
func Setup(t *testing.T, opts ...wasm.Option) *SimApp {
	t.Helper()
...
func SetupWithGenesisValSet(t *testing.T, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, opts []wasm.Option, balances ...banktypes.Balance) *SimApp {
	t.Helper()
...
// SetupWithEmptyStore set up a simd app instance with empty DB
...
func NewTestNetworkFixture() network.TestFixture {
	...
	app := NewSimApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, wasmtypes.EnableAllProposals, simtestutil.NewAppOptionsWithFlagHome(dir), emptyWasmOptions)
