...
func TestSimdExport(t *testing.T) {
	db := dbm.NewMemDB()
	gapp := NewSimAppWithCustomOptions(t, false, SetupOptions{
	...
	newGapp := NewSimApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, wasm.EnableAllProposals, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()), emptyWasmOpts)
